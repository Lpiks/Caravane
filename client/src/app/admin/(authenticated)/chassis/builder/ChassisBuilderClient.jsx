...
import { ArrowLeft, Save, Box as BoxIcon, Cylinder, Ruler, Palette, ShieldAlert, Layers, Plus, Trash2, Move, Minus, Copy, Check, ClipboardPaste, Triangle, Circle, RotateCw, ChevronDown, Type, Info } from 'lucide-react';
import ComponentPreviewCanvas from '../../../../../components/3d/ComponentPreviewCanvas';
import { useHistory } from '../../../../../hooks/useHistory';
import axios from 'axios';

export default function ChassisBuilderClient() {
  const [chassisName, setchassisName] = useState('Custom Chassis');
  ...
  const [chassisType, setchassisType] = useState('all');
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [previewRegion, setPreviewRegion] = useState('LHD');
  ...
  useEffect(() => {
    ...
    if (editId) {
      const loadChassis = async () => {
        ...
        try {
          const res = await axios.get(`${API_BASE}/api/chassis/${editId}`);
          const comp = res.data;
          if (comp) {
            setchassisName(comp.name);
            setchassisType(comp.chassisType || 'all');
            setFloorHeight(comp.floorHeight || 0);
            if (comp.states) {
          ...
          console.error(err);
          alert("Failed to load chassis details from the database.");
        }
      };
      loadChassis();
    }
  ...
  const handleSave = async () => {
    ...
    // Calculate exact bounding box for the entire custom chassis
    ...
    parts.forEach(p => {
      // S = 0.01 (cm to m)
      const sx = (p.size?.[0] || 0) * 0.01; // Width
      const sz = (p.size?.[1] || 0) * 0.01; // Length
      const sy = (p.size?.[2] || 0) * 0.01; // Height

      const ox = (p.offset?.[0] || 0) * 0.01;
      const oy = (p.offset?.[1] || 0) * 0.01;
      const oz = (p.offset?.[2] || 0) * 0.01;

      // Component preview centers Y around oy + sy/2
      const cx = ox;
      const cy = oy + sy / 2;
      const cz = oz;
    ...
    const lengthZ = isFinite(maxZ) ? (maxZ - minZ) * 100 : 100;
    const widthX = isFinite(maxX) ? (maxX - minX) * 100 : 100;
    const heightY = isFinite(maxY) ? (maxY - minY) * 100 : 100;

    const centerX = isFinite(maxX) ? (maxX + minX) / 2 : 0;
    const centerZ = isFinite(maxZ) ? (maxZ + minZ) / 2 : 0;

    const payload = {
      id: (editId && editId.startsWith('custom-')) ? editId : 'custom-' + Date.now(),
      name: chassisName || 'Custom Chassis',
      type: 'static',
      chassisType,
      floorHeight,
      defaultL: lengthZ,
    ...
    const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000';
    try {
      const finalId = editId || 'custom-' + Date.now();
      payload.id = finalId;

      await axios.put(`${API_BASE}/api/chassis/${finalId}`, payload);
      alert(`Successfully saved "${payload.name}" to the database for ${chassisType}!`);
    } catch (e) {
      console.error(e);
      alert("Failed to save chassis blueprint to the database.");
    }
  };
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-white flex flex-wrap items-center gap-2">
              Studio of the Studio <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-400 text-[10px] font-bold uppercase tracking-widest rounded border border-indigo-500/30">Chassis Builder</span>
            </h1>
            <p className="text-xs sm:text-sm text-slate-400">Procedurally build base chassis specifications.</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <div className="relative">
            ...
            >
              ...
              <span className="font-semibold text-white">
                {previewRegion === 'LHD' ? 'EU' : 'UK'}
              </span>
              <ChevronDown size={14} className={`text-slate-400 transition-transform ${isRegionDropdownOpen ? 'rotate-180' : ''}`} />
            </button>

            {isRegionDropdownOpen && (
              <div className="absolute top-full right-0 mt-2 w-32 bg-[#181A1D] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="p-1 flex flex-col">
                  {[
                    ...
                    <button
                      key={region.id}
                      onClick={() => { setPreviewRegion(region.id); setIsRegionDropdownOpen(false); }}
                      className={`px-3 py-2 rounded-lg text-xs font-semibold text-left transition-colors flex items-center justify-between ${previewRegion === region.id
                        ? 'bg-sky-500/10 text-sky-400'
                        : 'text-slate-300 hover:bg-white/5 hover:text-white'
                        }`}
                    >
                      {region.label}
                      {previewRegion === region.id && <Check size={14} className="text-sky-400" />}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="relative">
            ...
            >
              ...
              <span className="font-semibold text-white">
                {chassisType === 'all' ? 'All' :
                  chassisType === 't3' ? 'Compact' :
                    chassisType === 'l3h2' ? 'Standard' : 'Maxi'}
              </span>
              <ChevronDown size={14} className={`text-slate-400 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
            </button>

            {isDropdownOpen && (
              <div className="absolute top-full right-0 mt-2 w-44 bg-[#181A1D] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="p-1 flex flex-col">
                  {[
                    ...
                    <button
                      key={chassis.id}
                      onClick={() => { setchassisType(chassis.id); setIsDropdownOpen(false); }}
                      className={`px-3 py-2 rounded-lg text-xs font-semibold text-left transition-colors flex items-center justify-between ${chassisType === chassis.id
                        ? 'bg-sky-500/10 text-sky-400'
                        : 'text-slate-300 hover:bg-white/5 hover:text-white'
                        }`}
                    >
                      {chassis.label}
                      {chassisType === chassis.id && <Check size={14} className="text-sky-400" />}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <button
            ...
            className="bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/20 hover:border-indigo-500/40 font-bold px-3 py-2 rounded-lg text-xs flex items-center transition-all"
          >
            Import AI
          </button>
          ...
          <button
            onClick={handleSave}
            className="bg-sky-500 hover:bg-sky-400 text-sky-950 font-bold px-4 py-2 rounded-lg text-xs flex items-center gap-1.5 transition-transform active:scale-95 shadow-[0_0_15px_rgba(56,189,248,0.2)]"
          >
            <Save size={14} />
            <span>Save</span>
          </button>
        ...
        <div className={`w-full lg:w-[42rem] flex flex-col lg:flex-row gap-6 shrink-0 h-full overflow-hidden ${
          ...
          <div className={`w-full lg:w-64 flex flex-col gap-4 pr-2 pb-8 h-full min-h-0 ${
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 space-y-3">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Chassis Blueprint</label>
              <input
                type="text"
                value={chassisName}
              ...
              <div className="flex items-center justify-between pt-2">
                <div className="flex items-center gap-1.5 group relative">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Floor Height (Y)</label>
                  <Info size={14} className="text-slate-500 cursor-help hover:text-sky-400 transition-colors" />
                  ...
                  <div className="absolute top-full left-0 mt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    <div className="bg-[#181A1D] border border-white/10 rounded-lg p-3 shadow-2xl text-[10px] text-slate-300 leading-relaxed">
                      <div className="text-sky-400 font-mono mb-1">Formula:</div>
                      (Offset Y + Height Y) ÷ 100 = Floor Height in meters.<br />
                      <span className="text-slate-500 mt-1 block">e.g. Offset 28 + Height 10 = 38cm = <strong>0.38m</strong></span>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    step="0.01"
                    className="w-20 bg-[#0B0C10] border border-white/10 rounded-md px-2 py-1 text-center font-mono text-xs text-sky-400 focus:outline-none focus:border-sky-500/50 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                  />
          ...
          <div className={`flex-1 flex flex-col gap-5 overflow-y-auto pr-2 pb-8 custom-scrollbar ${
            ...
            {/* Shape Toggle / Quick Add Toggle */}
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <div className="flex items-center gap-2">
                  <BoxIcon size={16} className="text-slate-400" />
                  <h2 className="text-sm font-semibold text-white">
                    {showQuickAdd ? 'Quick Add Library' : 'Primitive Shape'}
                  </h2>
                </div>
                <button
                  onClick={() => setShowQuickAdd(!showQuickAdd)}
                  className="px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider bg-white/5 hover:bg-white/10 text-slate-300 transition-colors border border-white/10"
                >
                  {showQuickAdd ? 'Edit Shape' : 'Quick Add'}
                </button>
              </div>
              ...
              {showQuickAdd ? (
                <div className="flex overflow-x-auto gap-3 pb-2 custom-scrollbar snap-x">
                  <button
                    onClick={addPart}
                    className="shrink-0 w-[calc(33.33%-0.5rem)] snap-start flex flex-col items-center gap-1.5 p-3 rounded-lg bg-[#0B0C10] border border-white/5 hover:border-sky-500/50 hover:bg-sky-500/10 text-slate-400 hover:text-sky-400 transition-colors"
                  >
                    <BoxIcon size={20} />
                    <span className="text-[10px] font-semibold">Base Box</span>
                  </button>
                  <button
                    onClick={addWheel}
                    className="shrink-0 w-[calc(33.33%-0.5rem)] snap-start flex flex-col items-center gap-1.5 p-3 rounded-lg bg-[#0B0C10] border border-white/5 hover:border-emerald-500/50 hover:bg-emerald-500/10 text-slate-400 hover:text-emerald-400 transition-colors"
                  >
                    <Circle size={20} />
                    <span className="text-[10px] font-semibold">Wheel</span>
                  </button>
                  <button
                    onClick={addWindow}
                    className="shrink-0 w-[calc(33.33%-0.5rem)] snap-start flex flex-col items-center gap-1.5 p-3 rounded-lg bg-[#0B0C10] border border-white/5 hover:border-indigo-500/50 hover:bg-indigo-500/10 text-slate-400 hover:text-indigo-400 transition-colors"
                  >
                    <BoxIcon size={20} />
                    <span className="text-[10px] font-semibold">Window</span>
                  </button>
                  <button
                    onClick={() => {
                      const newId = `part-${Date.now()}`;
                      setParts([...parts, { id: newId, name: 'New Text', shape: 'text', textString: 'Text', size: [10, 10, 10], offset: [0, 0, 0], color: '#ffffff' }]);
                      setActivePartId(newId);
                    }}
                    className="shrink-0 w-[calc(33.33%-0.5rem)] snap-start flex flex-col items-center gap-1.5 p-3 rounded-lg bg-[#0B0C10] border border-white/5 hover:border-amber-500/50 hover:bg-amber-500/10 text-slate-400 hover:text-amber-400 transition-colors"
                  >
                    <Type size={20} />
                ...
                <div className="flex overflow-x-auto gap-3 pb-2 custom-scrollbar snap-x">
                  ...
                  <button
                    onClick={() => updateActivePart({ shape: 'text' })}
                    className={`shrink-0 w-[calc(33.33%-0.5rem)] snap-start flex flex-col items-center justify-center gap-2 py-3 rounded-xl border transition-all ${activePart.shape === 'text'
                      ? 'bg-sky-500/10 border-sky-500/30 text-sky-400'
                      : 'bg-[#0B0C10] border-white/5 text-slate-500 hover:border-white/10 hover:text-slate-300'
                      }`}
                  >
