...
const AnimatedPart = ({ part, index, isActive, parentPosition, parentRotation, parentH }) => {
  ...
  const getArrayValue = (field) => {
    ...
    if (typeof part[field] === 'object') return part[field]['default'] || [0, 0, 0];
    return [0, 0, 0];
  };
...
// Scene Wrapper
const Scene = ({ activeIndex, templates, dbChassis, dbComponents }) => {
  const turntableRef = useRef();
  const cameraRef = useRef();

  const getComponentParts = (typeId) => {
    const comp = dbComponents.find(c => c.id === typeId);
    return comp ? { parts: comp.parts, h: comp.defaultH } : { parts: [], h: 0 };
  };
  ...
  }, [activeIndex, templates, dbChassis]);
