...
export default function ShowroomControls() {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex fixed top-24 right-8 z-30 flex-col gap-5 items-end">
        ...
        <div className="flex flex-col items-end gap-1.5">
          ...
          <button 
            ...
            className={`p-3.5 rounded-full backdrop-blur-md shadow-lg border transition-all duration-300 hover:scale-105 active:scale-95 ${
              headlightsOn 
      ...
      <div className="md:hidden fixed top-20 right-4 z-30 flex flex-col items-end gap-2.5">
        <button
          onClick={() => setIsMobileControlsOpen(!isMobileControlsOpen)}
          className="p-3 rounded-full bg-obsidian/85 backdrop-blur-md text-white border border-white/20 shadow-xl flex items-center gap-1.5 active:scale-95 transition-transform"
        >
        ...
        {/* Floating Scroll Down Button (Mobile) - Bouncing below the Controls button */}
        {!isMobileControlsOpen && (
          <button
            ...
            className="w-11 h-11 rounded-full bg-obsidian/85 backdrop-blur-md border border-white/20 text-terracotta flex items-center justify-center shadow-xl active:scale-95 transition-transform animate-bounce"
            title="Scroll Down"
          >
            <ChevronDown size={20} />
          </button>
        ...
        {isMobileControlsOpen && (
          <div className="flex flex-col gap-3 p-3 bg-obsidian/90 backdrop-blur-2xl border border-white/20 rounded-2xl shadow-2xl animate-in fade-in zoom-in-95 duration-150">
            ...
            <button
              onClick={toggleHeadlights}
              className={`flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider transition-colors ${
